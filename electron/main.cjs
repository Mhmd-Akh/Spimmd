...
if (fs.existsSync(CACHE_CONFIG)) {
  try {
    maxCacheSize =
      JSON.parse(fs.readFileSync(CACHE_CONFIG, "utf8")).maxSize || 500;
  } catch {}
}

function getCacheIndex() {
  try {
    return fs.existsSync(CACHE_INDEX)
      ? JSON.parse(fs.readFileSync(CACHE_INDEX, "utf8"))
      : {};
  } catch {
    return {};
  }
...
function getPlayCounts() {
  try {
    return fs.existsSync(PLAY_COUNT_PATH)
      ? JSON.parse(fs.readFileSync(PLAY_COUNT_PATH, "utf8"))
      : {};
  } catch {
    return {};
  }
...
function savePlayCounts(counts) {
  fs.writeFileSync(PLAY_COUNT_PATH, JSON.stringify(counts, null, 2));
}

function getTotalCacheSize() {
  let total = 0;
  Object.values(getCacheIndex()).forEach((e) => (total += e.size || 0));
  return total;
}

function cleanOldestCache(neededSpace) {
  const index = getCacheIndex();
  const entries = Object.entries(index).sort((a, b) => a[1].date - b[1].date);
  let freed = 0;
...
autoUpdater.setFeedURL({ provider: "generic", url: UPDATE_SERVER + "/" });
...
function readSecureData() {
  try {
    return fs.existsSync(CONFIG_PATH)
      ? JSON.parse(decrypt(fs.readFileSync(CONFIG_PATH, "utf8")))
...
ipcMain.handle("set-title", (e, t) => {
  if (mainWindow) mainWindow.setTitle(t);
});

// 🔥 Cache
ipcMain.handle("read-cached-file", (e, fp) => {
  try {
    return {
      success: true,
      data: Array.from(new Uint8Array(fs.readFileSync(fp))),
    };
  } catch {
...
ipcMain.handle("set-play-count", (e, id, c) => {
  const p = getPlayCounts();
  p[id] = c;
  savePlayCounts(p);
  return true;
});

ipcMain.handle("cache-audio", (e, { trackId, audioBuffer, title, artist }) => {
  try {
    const filePath = path.join(CACHE_DIR, `${trackId}.opus`);
    const buffer = Buffer.from(audioBuffer);
    const totalSize = getTotalCacheSize();
    if (totalSize + buffer.length > maxCacheSize * 1024 * 1024)
      cleanOldestCache(buffer.length);
...
ipcMain.handle("get-cached-audio", (e, trackId) => {
  const opusPath = path.join(CACHE_DIR, `${trackId}.opus`);
  const mp3Path = path.join(CACHE_DIR, `${trackId}.mp3`);
  const filePath = fs.existsSync(opusPath) ? opusPath : mp3Path;
  if (fs.existsSync(filePath))
    return { exists: true, path: filePath, size: fs.statSync(filePath).size };
  return { exists: false };
});
...
ipcMain.handle("set-max-cache-size", (e, s) => {
  maxCacheSize = s;
  fs.writeFileSync(CACHE_CONFIG, JSON.stringify({ maxSize: s }));
  if (getTotalCacheSize() > s * 1024 * 1024)
    cleanOldestCache(getTotalCacheSize() - s * 1024 * 1024);
  return { success: true };
});
...
ipcMain.handle("check-update", async () => {
  try {
    const r = await fetch(UPDATE_INFO_URL);
...
autoUpdater.on("update-available", (i) => {
  if (mainWindow) mainWindow.webContents.send("update-available", i);
});
autoUpdater.on("download-progress", (p) => {
  if (mainWindow) mainWindow.webContents.send("download-progress", p.percent);
});
...
function createWindow() {
  ...
  if (isDev) {
    ...
    const server = http.createServer((req, res) => {
      ...
      let fp = path.join(
        distPath,
        pu.pathname === "/" ? "index.html" : pu.pathname,
      );
